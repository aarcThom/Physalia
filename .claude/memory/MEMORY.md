...
- [Merge Signal join](merge-signal-join.md) — 2026-08-16: new Control Flow component merging 2+ signals into one. It is a **JOIN, not a passthrough** (parallel branches latch on separate scheduled solves, so per-solve merging would emit once per branch); variable inputs append/remove at the **END only**, because the hold and the base's consume-once `_marks` are index-keyed.
...
- [Group-scoped grounding](group-scoped-grounding.md) — 2026-07-28: master "Physalia" group auto-enrolls every LLM placement; new "Physalia Group Components" grounder exports only its contents (patch frame resolved by checksum MATCHING — a `sha256-group-` prefix broke the library's schema regex, hard lesson recorded); plug-in tabs unchecked by default; verbosity sweep. **Proven live** — 5-stage White House build completed.
