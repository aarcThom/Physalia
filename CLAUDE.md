...
- **A harness has one INLET per Receiver inside it** — its only kind of input, and the mirror of the
  ...
  (hidden generic param, **tree access**, optional) per Receiver, sharing ONE nickname with that
  Receiver's OUTPUT parameter — both start "Data", and renaming either end renames the other (the
  Receiver's own node nickname is not involved and stays free to say what the node is). **Bound by `InstanceGuid`, never by position** — and this is where the outlet pattern must
  ...
  **move** change what the proxy must show and reach no solution anywhere, the same class of problem
  ...
  `Param_LinkedName` base: `Param_ReceiverData` (the Receiver's output) relabels the input via
  `OnInletRenamed`, and `Param_Inlet` renames the output via `RenameReceiver` — one name, either end
