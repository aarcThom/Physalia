...
- [Memory tool](memory-tool.md) — provider-agnostic Anthropic-style `memory` tool (view/create/str_replace/insert/delete/rename) + `MemoryGrounding` opt-in switch + `/m/global`//`/m/local` chat refs; global + per-`.gh`-file scopes under `Files/memories`. 2026-07-02, builds+tests clean, live test pending.
