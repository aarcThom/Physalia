...
- **[Data folder + update notice](data-folder-and-update-notice.md)** — 2026-09-09, **update dialog RUN LIVE in Rhino**, migration headless-only: user files → `%LOCALAPPDATA%/Physalia`, shipped content OVERLAID. **Read before yak packaging.**
...
- [Teaching presets](teaching-presets.md) - 2026-09-09, **28** annotated .phy harnesses, now SHIPPED in `Files/PRESETS/Physalia/AI` behind the pink Experimental toggle (`wip_presets/` gone), generated from tools/presets/: fourteen that teach the PARTS (all run live) and fourteen SCENARIO ones that teach the WORK. **C# Transmitter run live for the first time** (S05) and the guardrail chain again (S04). Three new topologies: two Conversation Logs in one harness joined by ONE wire, two nested harnesses, and a VALUE rather than prose on a Harness Out. Provider facts still hold — Claude Code cannot call tools; and a **broken PATH inside Rhino looks exactly like a broken Codex install**.
- [Preset conventions](preset-conventions.md) — **read before building a preset.** Naming, `Files/PRESETS/User` vs `Physalia`, the tools-capable model rule, Picker discipline, the wireless backward hops, and the STORED-vs-LOADED check. Presets live beside the PLUG-IN (`<assembly>/Files`), so a generated one is **not version-controlled** and an un-staged one is not listed.
- [Driving Rhino from WSL](driving-rhino-from-wsl.md) — the working script channel (SendKeys, not `RhinoCode`), why it silently stops (Grasshopper steals focus; a modal dialog blocks everything after it), the **two Python engines**, the `RhinoApp.Idle` trap, and the PATH a WSL-launched Windows process actually gets (`uvx` yes, `npx` no).
- [Harness Builder meta-preset](harness-builder-preset.md) — 2026-09-07, RUN END TO END: a harness that WRITES harnesses (it generated an 18-object preset that then loaded stably). **FOUR Physalia component names are ambiguous** — `Read PDF` is both an LLM tool and a human tool — so resolve by name AND ribbon section.
- [ComfyUI render preset](comfy-render-preset.md) — 2026-09-07, RUN END TO END (viewport → SDXL img2img → image back in the chat, ~19s on a 4060 Ti). **comfy-mcp wraps comfy-cli, so `uvx comfy-mcp` on Windows lists 39 tools then fails every call**; it must run in WSL.
- [PDF natives verified](pdf-natives-verified.md) — 2026-09-07, RUN IN RHINO: `PdfNativeLibrary` resolves, full-page and region renders correct (images inspected). The node reads **`<project>/PDF`**, and a manual tool call cannot show you its own return value.
- [MCP bridge + OAuth](mcp-integration.md) — 2026-09-07, RUN IN RHINO: the bridge connects a remote Streamable HTTP server beside a local stdio one and `tools/call` round-trips. OAuth reaches the browser with dynamic client registration; only the token exchange needs a human.
- [Blender MCP preset](blender-mcp-preset.md) — 2026-09-07, RUN LIVE (cube created in Blender through the pipeline). **The Claude Code provider IGNORES `tools`**, so it can never drive a Router — the pipeline stays green while being incapable. Rhino's `PYTHONHOME` kills any Python MCP server.
- [Closing Grasshopper reaches nothing](gh-custom-attribute-traps.md) — 2026-09-07, harness panel RUN LIVE (chat half built, not yet run): the panel and the chat window outlived the GH window. **GH cancels its own close and hides** (so `FormClosed` never fires), and **WinForms never raises `VisibleChanged` on a child when an ANCESTOR is hidden** — so follow the host FORM's visibility. Both windows now come BACK when GH reopens: away on the GESTURE, back on the editor's visibility. Hiding rather than closing needed `ChatWindow.CanAskUser`, or every gate that fails closed on `ActiveWindow is null` posts a card nobody can see.
- [Pre-ship testing pass](pre-ship-testing-pass.md) — 2026-09-06/07. **A1–A6, B0–B3, B7, C1, C2 RUN LIVE and passing**, incl. the `PipelineWake` re-enable that had never been exercised. Two defects found, fixed and RE-VERIFIED against the fixed build. **F3 is the only ship blocker left** — drive it with `tools/overnight/Watch-OvernightRun.ps1`. Two lessons: a rig can pass every assertion it states and still be broken (C2 did), and a bug's MASKED copy is the one that survives a pass. **C3 rewritten 09-07** — a feature landing after the pass changed what it asserts.
...
- [Building harnesses programmatically](building-harnesses-programmatically.md) — extended 2026-09-06/07: **`SetPersistentData` APPENDS** to the registered default (two items on an item input solves the component twice), **a string becomes one item per CHARACTER**, placing a harness points the canvas INTO it, and a scripted harness comes up with no Chat. Now also the DOCUMENT layer — `canvas.set_Document()`, `AddNewDocument()`, compare by `DocumentID` never `is`, and `GH_DocumentIO.Paste` silently does nothing from a script. The backward-path rule was already written down — read it before wiring.
...
- [Trigger tier](trigger-tier.md) — 2026-09-06, Watch Modelling RUN LIVE, other four built-not-run: Physalia can finally START a round on its own (Timer / Folder Watcher / Rhino Changed / Data Changed). **Arming is session-only and never serialized**, `PipelineWake` stops a wake-up being silently dropped on a harness sub-document, and `PipelineFileWrites` breaks the download→watcher→download loop.
- [Signal conditionals](signal-relay-conditionals.md) — 2026-09-06, **ALL FIVE RUN LIVE**: `SignalRelayBase` (Gate / Hold / Switch / Throttle) plus For Each. **A relay forwards the ORIGINAL signal** — a re-mint would strip the Instructions a gate on that hop exists to pass through.
- [Declare + Ask Human](intent-tools-declare-ask-human.md) — 2026-09-06: the model picks a route the pipeline offers, and can ask a question (including "select these in Rhino"). **Every question edge returns UNANSWERED, never a guessed default** — that is why its broker is a third sibling rather than part of the approval one.
- [Harness delegation](harness-delegation.md) — 2026-09-06, **RUN LIVE** (drive it with Construct Tool Call, no LLM needed): a harness as a TOOL of another harness, so a subtask stops living in the caller's one conversation. Paired on the inner GH_Document with no wire; one task at a time; refuse up front rather than time out.
- [Session budget + records](session-budget-and-records.md) — 2026-09-06, **autosave + run log VERIFIED ON DISK**: Budget Guard bounds a SESSION (nothing else did), `conversation.json` autosaves per turn and resumes only on request, `runs.jsonl` logs one line per call. **On a CLI provider a token cap is MEANINGLESS** — it reports the delta it was sent, not the prompt, so `Max Calls` is the only cap that bounds one.
- [Model API credentials](model-api-credentials.md) — 2026-09-04/09-06, BUILT not run in Rhino: providers configured in the chat window, DPAPI-encrypted; endpoint+key on one `GH_ModelApi` wire (**new GUID**); both YAML config files deleted. **09-06: a configured provider was unreachable** — its pill is now a door, the connected page reconfigures (blank key = KEEP, never clear) and every connection can be switched off, Claude Code included.
- [Harness names + .phy packages](harness-names-and-phy-packages.md) — 2026-09-05, BUILT not run in Rhino: harnesses are named `curious-cake-soap-fun` (DERIVED from the guid), own `Files/PROJECT_FILES/<name>/`, and save as `.phy`. A rename MOVES the folder; the key is stored with its owning guid or a paste steals the original's downloads. Harness Notes + the two pill widgets + `Files/PDFS` are DELETED — killing the notes component is what FORCED the package format. **09-07: Save .phy… added** (destination picked, project folder carried WHOLE) — the exported package and the preset differ in exactly that one flag, and `Physalia.GH` has no test project, so rig C3 is its only cover. Not run in Rhino.
- [Project file tools](project-file-tools.md) — 2026-09-05, Core tested / GH not run in Rhino: Project Folder grounding + `download_file` + `read_file`, and a fail-closed approval seam shown as a CARD in the chat window. The PATH on the wire is the deliverable. Sniff the bytes — an HTML error page saved as `.las` looks exactly like success. `read_file`'s containment is NOT a sandbox and must not be sold as one.
- [Chat link prompt + markdown restyle](chat-link-prompt.md) — 2026-09-05, fixed (headless-verified, not in Rhino): the external-link warning rendered unstyled over the conversation. **Tailwind never scans `node_modules`**, so streamdown's whole theme was dead here and its `window.open` confirm reached no browser — the chat now shows its OWN prompt and opens links through the host. Second pass added the `@source` line and a `theme` override, because compiling a library's theme is not the same as wanting it at page scale in a 460px panel.
...
- [New machine bring-up](new-machine-bringup.md) — 2026-09-07: a clean Debug solution build leaves `Bridge/` EMPTY (the slnx excludes McpBridge from `Debug|*`), so every REMOTE MCP server reports the bridge missing from a green build. Also: the dev loop is an undocumented `.ghlink` in `%APPDATA%\Grasshopper\Libraries`, SDK 10 alone builds every TFM, Node is a hard requirement, launching Rhino from WSL needs `SendKeys` because `/runscript` silently no-ops, and the README's install/config steps cannot be followed at all.
