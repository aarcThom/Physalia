...
## Latest
- [Group-scoped grounding](group-scoped-grounding.md) — 2026-08-17 additions: the CompTx **drag-out** creates the master "Physalia" group (holding a light robin-egg-blue hint panel, excluded from exports like the group itself), and **dragging that group carries the harness proxy + its wires along without the proxy being a member** — GH raises no "moved" event, so it is a throttled idle poll for a rigid member delta (`MasterGroupFollower`).
