...
**Feedback marker chain (non-obvious, spans Core→UI):** feedback and typed prompts are both `Role.User`, so a one-bit flag carries the distinction. `ConversationMessage.IsFeedback` (Core, init-only, default false, ignored by providers) ← set by `Conversation Log` only in its `InFeedbackSignal` case (`RecordUserText(payload, isFeedback:true)`) ← surfaced by `ChatWindow.BuildMessages` as `UiMessage.Feedback`/`feedback` (camelCase JSON) ← `App.svelte` passes `feedback` to `Message` ← which emits `is-feedback` (instead of `is-user`) ← `message-content.svelte` has a parallel `group-[.is-feedback]:` pink block. Chosen over text-matching the 5 feedback templates (Schema Validator/PyTransmitter/Component Resolver/ComponentTransmitter/Canvas Observation), which is fragile.
