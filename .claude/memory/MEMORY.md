...
- [Teaching presets](teaching-presets.md) - 2026-09-07, **28** annotated .phy harnesses in wip_presets/, generated from tools/presets/: fourteen that teach the PARTS (all run live) and fourteen SCENARIO ones that teach the WORK. **C# Transmitter run live for the first time** (S05) and the guardrail chain again (S04). Three new topologies: two Conversation Logs in one harness joined by ONE wire, two nested harnesses, and a VALUE rather than prose on a Harness Out. Provider facts still hold — Claude Code cannot call tools; and a **broken PATH inside Rhino looks exactly like a broken Codex install**.
