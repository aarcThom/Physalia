...
internal sealed class CodexSession : IDisposable
{
    ...
    /// <param name="tools">The tools to advertise, fixed for the life of the thread; null or empty for none.</param>
    public CodexSession(
        string modelId,
    ...
    /// <summary>
    /// Gets the tools advertised to the model, declared once when the thread is opened. Never null.
    /// </summary>
        ...
        [EnumeratorCancellation] CancellationToken ct)
    {
        ...
        try
        {
            ...
            // Tool calls the model made during this turn. They are NOT executed here — see
            // HandleToolCallAsync — they are handed back on the final chunk so Physalia's own
            // Router loop runs them, exactly as it does on the HTTP providers.
            ...
            while (true)
            {
                ...
                switch (parsed.Kind)
                {
                    case LineKind.ServerRequest:
                        // Nothing here implements the agent's other side-channel requests
                        // (approvals, elicitations). Answering with a JSON-RPC error declines them
                    ...
                    case LineKind.ToolCall:
                    {
                        toolCalls.Add(new LlmToolCall(parsed.ToolCallId!, parsed.ToolName!, parsed.Text ?? "{}"));

                        // A thinking block still open belongs to the run-up to the call, so close
                        // it here — nothing after this point is forwarded to close it later.
                        if (thinkingTagOpen)
                        {
                        ...
                        // Answer so the server stops waiting, then stop the turn. Deferring rather
                        // than executing is what keeps the Router the single place tools run: the
                        // call goes back to Physalia on the final chunk, its result returns as the
                        // next user turn, and the thread stays warm for it.
                        await SendToolDeferralAsync(parsed.RequestId, ct);
                    ...
                    case LineKind.TextDelta:
                    {
                        ...
                        // Everything the model says AFTER a tool call is a reaction to being told
                        // the call was deferred — an apology, or an offer to try again. The turn
                        // is over as far as Physalia is concerned (the Router now owns the call),
                        // and the interrupt does not always land before the model gets a sentence
                        // out, so the tail is dropped rather than raced for. What survives is the
                        // run-up text, which is exactly the assistant turn the HTTP providers
                        // produce: preamble plus tool_use blocks.
                    ...
                    case LineKind.ThinkingDelta:
                    {
                        ...
                        // Post-call reasoning is reasoning about the deferral. Dropped for the same
                        // reason as the text above.
                        if (toolCalls.Count > 0)
                        {
                            break;
                        }
                    ...
                    case LineKind.FinalText:
                        // Same rule: once a call is out, the completed message is the deferral
                        // reaction, never the answer.
                        if (toolCalls.Count == 0)
                        {
                    ...
                    case LineKind.TurnCompleted:
                    {
                        ...
                        yield return Ok(new LlmResponseChunk(
                            ...
                            toolCalls.Count > 0 ? "tool_calls" : parsed.Text));
                        yield break;
                    }
    ...
    private async Task EnsureStartedAsync(CancellationToken ct)
    {
        ...
        if (Tools.Count > 0)
        {
            threadParams["dynamicTools"] = BuildDynamicTools();
        }
    ...
    {
        // experimentalApi is opted into ONLY when there are tools to declare: `dynamicTools` is an
        // experimental field and this is the one thing here that needs it, so a plain text session
        // keeps the conservative handshake it was verified with.
        var initialize = new Dictionary<string, object?>
        {
            ["clientInfo"] = new { name = "physalia", version = "1.0.0", title = "Physalia" },
        };

        if (Tools.Count > 0)
        {
            initialize["capabilities"] = new { experimentalApi = true };
        }

        long id = await SendRequestAsync("initialize", initialize, ct);
    ...
    /// <summary>
    /// Renders Physalia's tool definitions as the CLI's flat function specs. Namespaces are not
    /// used: the Router dispatches on the bare tool name, so the name the model calls must be the
    /// name the tool node advertises.
    /// </summary>
    private object[] BuildDynamicTools()
    {
        var specs = new List<object>(Tools.Count);
        foreach (LlmToolDefinition tool in Tools)
        {
            ...
            specs.Add(new
            {
                ...
                inputSchema = ToolSchema.Parse(tool.InputSchemaJson),
            });
        }
    ...
    private object[] BuildUserInput(IReadOnlyList<MessageContent> content)
    {
        ...
        foreach (MessageContent block in content)
        {
            switch (block)
            {
                ...
                // A tool round comes back as TEXT rather than as a native tool result, and that is
                // deliberate. The model's call was already answered inside the turn it was made in
                // (with the deferral), so the thread has no open call to satisfy; what it needs now
                // is the answer, in the message the deferral promised. The wording matches
                // ConversationHelpers so a result reads the same here as it does in a reseed.
                case ToolResultContent result:
                    items.Add(new
                    {
                        type = "text",
                        text = $"[{(result.IsError ? "Tool error" : "Tool result")}: id:{result.ToolCallId}]\n{result.Content}",
                    });
                    break;

                // Defensive: an assistant-side call block has no business in a user turn, but a
                // compaction or repair pass could put one there, and dropping it silently would
                // leave the model looking at a result for a call it cannot see.
                case ToolCallContent call:
                    items.Add(new
                    {
                        type = "text",
                        text = $"[Tool call: {call.Name} (id:{call.Id})]\n{call.InputJson}",
                    });
                    break;
    ...
    /// <summary>
    /// The text the model is shown in place of a tool result. It has to say three things, because
    /// the model reads it and acts on it: the call was not run HERE, do not retry it, and the
    /// answer is coming as the next user message — which is exactly what happens once the Router
    /// has dispatched it.
    /// </summary>
    private const string ToolDeferralText =
        "Deferred: this tool runs outside the turn and was not executed here. Do not retry it. "
        + "Its result will be delivered to you in the next user message.";

    private Task SendToolDeferralAsync(long id, CancellationToken ct)
        => WriteMessageAsync(
            new Dictionary<string, object?>
            {
                ["jsonrpc"] = "2.0",
                ...
                // success:false because it is true — nothing ran. Claiming success and returning no
                // output would read to the model as a tool that answered with nothing.
                ["result"] = new
                {
                    success = false,
                    contentItems = new[] { new { type = "inputText", text = ToolDeferralText } },
                },
            },
            ct);

    private Task SendInterruptAsync(CancellationToken ct)
        => WriteMessageAsync(
            new Dictionary<string, object?>
            {
                ["jsonrpc"] = "2.0",
    ...
    private static ParsedLine ParseLine(string raw)
    {
        ...
        try
        {
            ...
            if (hasId)
            {
                long requestId = idEl.GetInt64();
                if (methodEl.GetString() == "item/tool/call"
                    && root.TryGetProperty("params", out JsonElement callParams))
                {
                    string? callId = ReadString(callParams, "callId");
                    string? tool = ReadString(callParams, "tool");

                    // A call missing either identifier cannot be dispatched or matched to a result,
                    // so it is declined like any other unimplemented request rather than being
                    // handed to the Router as a half-formed call.
                    if (!string.IsNullOrEmpty(callId) && !string.IsNullOrEmpty(tool))
                    {
                        // `arguments` is free-form JSON; Physalia carries tool input as a string.
                        string arguments = callParams.TryGetProperty("arguments", out JsonElement args)
                            && args.ValueKind is not (JsonValueKind.Undefined or JsonValueKind.Null)
                                ? args.GetRawText()
                                : "{}";
                ...
                return new ParsedLine(LineKind.ServerRequest, RequestId: requestId);
            }
