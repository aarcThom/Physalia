...
## Latest
- [Pre-ship testing pass](pre-ship-testing-pass.md) — 2026-09-06, the off-canvas half of `planning/pre-ship-testing.md` run green (build, 964 Core tests, 11 headless UI rigs). Two findings: **`StateStore.All()` duplicated a key on a case-variant re-set** (fixed, `final-pass`), and the icon debt is **29** components, not 18. Arming cannot serialize — no trigger overrides `Write`/`Read` at all.
