...
- The triplicated grip + wire-cache + drag state machine (was duplicated across `GripLinkAttrib`, `CompTxAttrib`, `ChatboxAttrib`) now lives in one **`ArrowGrip`** controller, driven by an **`IArrowHost`** interface (`ArrowOrigin`, `ArrowGradient`, `ArrowHead`, `HorizontalArrow`, `SettledEndpoints(doc)`, `OnDrop(doc,pt,ctrl)`). Used by **composition** because `ChatboxAttrib` can't change its `GH_ComponentAttributes` base. Each host keeps only its own hit-test in `RespondToMouseDown` then calls `_arrow.StartDrag/UpdateDrag/EndDrag`; `IsDragging` gates Move/Up.
- `ChatboxAttrib`'s old `_wires`/`WireAt`/`DrawArrowWires` are gone (folded into `ArrowGrip`); its harness tint/glow (`RenderSmoothCapsule`/`DrawHarnessGlow`/`LinearGradientBrush`) is untouched — not arrow code.
...
- `FeedbackCollectorAttrib` is now ~10 lines (just `: BottomGripAttributes`). `PickerAttrib` and `ChatboxAttrib` rebased on `PhyComponentAttributes` (use `IsHarnessCollapsed`, no direct CollapseGuard). `ChatboxAttrib` stays a bespoke proxy (own `CanvasGrip`, composition `ArrowGrip`).
- **`ArrowGrip` no longer owns/draws the grip dot** — it's wires + drag only; the grip is drawn by `BottomGripAttributes` (or Chatbox itself). Grip-draw helper pattern: subclass override `DrawGrip`/`RenderGripContent`.
