...
/// </summary>
public class PyTransmitterAttrib : GH_ComponentAttributes
{
    ...
    protected override void Render(GH_Canvas canvas, Graphics graphics, GH_CanvasChannel channel)
    {
        ...
        if (channel == GH_CanvasChannel.Wires)
        {
            ...
            var linkedGuid = _pyTransmitter.LinkedGuid;
            ...
            foreach (var obj in sender.Document.Objects)
            {
                if (obj.Attributes.Bounds.Contains(e.CanvasLocation) && GhPythonBridge.IsScriptComponent(obj))
                {
                    if (_isConnecting)
                    {
                        ...
                        _pyTransmitter.LinkTo(obj.InstanceGuid);
                    }
                    else
                    {
                        _linkedWire = null;
                        _pyTransmitter.Unlink();
                    }

                    _pyTransmitter.ExpireSolution(true);
                    break;
                }
            }
