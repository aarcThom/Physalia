...
**Image gating** — image intake FULLY disabled unless Add Image wired: Composer gates addImages choke point, drag-drop listeners detach reactively, paste file branch skipped, pending pills purged if unwired mid-composition; C# `SubmitJsonPayload` defensively drops images too. **Button visibility (Thomas's call):** Add-image icon HIDDEN entirely until the tool is wired (then clickable); geometry-snapshot icon APPEARS as soon as its tool is wired but stays greyed/unclickable until generated geometry exists (Composer takes `snapshotWired` + `snapshotArmed` separately). New state field `imageToolWired` (in BOTH Tick groundingSignature and state). Grounding panel kinds view gained a separated "Human Tools" section (snapshot pill moved there + read-only "Image attachments — Enabled" row).
