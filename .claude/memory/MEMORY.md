...
- [Commit/PR messages output-only](commit-and-pr-messages-output-only.md) — print them in chat by default; `git commit` only on an explicit instruction, never `push`/`gh`. **That instruction covers THAT batch, not the rest of the session.**
- [Never `git checkout` a file to undo your own edit](git-checkout-discards-session-work.md) — mid-session it is uncommitted, so it reverts to HEAD and destroys the whole session's work on that file. Cost the entire setup-page rework once.
...
## Latest
- [Model API credentials](model-api-credentials.md) — 2026-09-04/05, **BUILT, committed, not yet run in Rhino**: providers are set up in the chat window and stored DPAPI-encrypted; endpoint+key collapsed into one `GH_ModelApi` wire (OpenAI-compatible node lost Base URL, **new GUID** — the Local LLM preset needs re-saving). **Both YAML config files deleted outright.** Three per-user stores now: `credentials.dat` (encrypted), `providers.json` (the opt-in list), `mcp-servers.json`. Rules: **encrypt what the machine writes, not what the human writes**; **availability is not consent** (a found key/CLI is offered, never adopted); `ISecretStore` is the ONE platform seam, so **macOS Keychain is one class + one line**.
- [MCP bridge vs Illustrator](mcp-bridge-chunked-body.md) — 2026-09-04: chunked POST bodies broke Adobe's MCP server (it parsed the chunk-size as JSON); a 404 on the optional GET stream killed the session; our bridge exited 0 saying nothing. Fixed + `--trace`.
- [MCP setup page + the YAML's removal](mcp-setup-page.md) — 2026-09-04/05: landing page (paste a `claude`/`codex` CLI command, or fill the form) + `McpCommandParser`. **2026-09-05: `MCP_SERVERS.YAML` is GONE** — servers live in `%LOCALAPPDATA%/Physalia/mcp-servers.json` as the standard `mcpServers` block; `McpConfigEditor` and the `.example` went with it. **An import deletes the file it read ONLY when something parsed** — otherwise it is a deletion, not a migration.
- [run_rhino_script tool](run-rhino-script-tool.md) — 2026-09-03, **BUILT and WORKING live**: the model runs Python against the live Rhino doc, so **`print` IS the read-back** and no `get_context` tool ships. Trap: **RhinoCodePlugin loads ON DEMAND** — force `LoadPlugIn` or a cold registry has no Python.
- [Building harnesses programmatically](building-harnesses-programmatically.md) — 2026-09-02: build a pipeline inside a harness from code via `HarnessComponent.EnsureInnerDocument()` + `ComponentServer.EmitObject`. **Every backward path must be wireless** or the Conversation Log reports a recursive data stream. An "empty" harness already holds a Chat.
- [Rhino MCP vs native control](rhino-mcp-vs-native-control.md) — 2026-09-02: **decision record.** Shipping McNeel's Rhino MCP as a preset is the WRONG path — its router path is version-stamped, so it cannot travel in a preset. Also: **`_router_quit_app` is on their public endpoint** (filter `_router_`).
- [MCP integration](mcp-integration.md) — 2026-08-27, **BUILT**: Physalia as an MCP **client**. **Measured: the official C# SDK cannot run in-process** (Rhino's shared framework serves `System.Text.Json`, so no shipped DLL can fix it) — hence stdio in-process + the `Physalia.McpBridge` relay for remote/OAuth.
...
- [Headless chat-UI testing](headless-chat-ui-testing.md) — 2026-08-21, extended 2026-09-05: drive `dist/index.html` in headless Chrome, no Rhino. Traps: `</body>` occurs inside the inlined JS (use rpartition), and never assert on the DOM in the click’s own tick. **It measures LAYOUT too** — drive it at the REAL 460x620 window size, and compare rects against the inner SCROLLER, not `window.innerHeight`.
...
- [Harness contents arrive unsolved](harness-subdocument.md) — 2026-08-21: **nothing on the host canvas solves the sub-document** — expiring the proxy runs the HOST's solution. New `HarnessComponent.PrimeInner()` covers preset/empty placement, file load, paste and Load-from-.gh with one hook.
- [Doc-comment validation is on](physalia-repo-gotchas.md) — 2026-08-21: `GenerateDocumentationFile` enabled on all three projects, so the COMPILER checks every cref/param tag. It had never been on; switching it on found 16 silent defects in one sweep. See also [[gh-custom-attribute-traps]].
...
- [Tool image attachments](tool-image-attachments.md) — 2026-08-18: new `take_snapshot` tool. **A tool result is TEXT on every provider**, so an image rides the answering user turn as an ATTACHMENT, ordered AFTER every `ToolResultContent` (Anthropic needs those first). It was being dropped in two silent places.
- [Move In Space tool](move-in-space-tool.md) — 2026-08-17: new `move_in_space` tool. Adjacency is DERIVED (8 in-plane cones × 3 vertical bands, closest-per-bucket); world-frame directions, not heading-relative. New `LlmToolComponentBase` hooks `RegisterAdditionalOutputs` + `OnSolveEnd`.
- [Harness sub-document](harness-subdocument.md) — 2026-08-17: **the residency guard is GONE** (components may sit bare on the canvas; only the transmitter drag arrow broke, restored by `OutletArrowAttrib` reading residency LIVE). Also **"Load Harness from .gh File…"** — whose lesson is the teardown: a discarded sub-document must be `RemoveObjects` + `Dispose`d or its `RemovedFromDocument`s never run and warm CLI sessions leak; ADOPT FIRST.
- [Feedback turn attribution](feedback-turn-attribution.md) — 2026-08-17: pink feedback turns collapse behind a header wearing the producing component's icon + nickname. Needed `PhySignal.Origins` **provenance** (not a carrier field), because every aggregator re-mints under its own identity and erased the producer.
- [Group-scoped grounding](group-scoped-grounding.md) — 2026-07-28: the master Physalia group auto-enrolls every LLM placement (per harness since 2026-08-17); patch frame resolved by checksum MATCHING. **Proven live** — 5-stage White House build completed.
...
- [Codex dynamic tools](codex-dynamic-tools.md) — 2026-08-16: Codex calls Physalia's LLM Tools with **zero canvas changes** — declared at thread start, the call DEFERRED back to the Router. Load-bearing: drop all text after a tool call, because `turn/interrupt` loses the race.
...
- [Picker selection persistence](picker-ghjson-serialization.md) — `physalia.pickerValue` for GhJSON (labels only, never secrets) + the **provisional-list trap**: a Picker solves BEFORE its source, so a seed list ate restored picks until `PickableInput.IsSettled`.
...
- [GH_ApiKey goo](gh-apikey-goo.md) — **SUPERSEDED 2026-09-04 by [[model-api-credentials]]**: `GH_ApiKey`/`Param_ApiKey`/the API Keys component are DELETED, replaced by `GH_ModelApi` (provider + endpoint + key). The label-only, never-serialized discipline carried over intact.
