...
**Deleted** (+ dead code): `Prompter` (Core/Prompter.cs) and its `Attributes/PrompterAttrib.cs` — Chatbox is now the sole prompt entry point; `PythonShortcut`; `SchemaTranslator`. Also removed icons Prompter.png/SchemaTranslator.png. Stale `Prompter` comments across ~10 files rewritten to Chatbox. `PromptPipelineView` kept (shared, now Chatbox-only).

**Sections** = the GH **subCategory** string (the C# namespaces are flat `Physalia.GH.Components` — folders are just org). New folders + subCategories: `TokensCompaction`="Tokens & Compaction" (TokenEstimator, TokenizationTechniques, + the 6 compaction comps; note TokenThreshold & Summarizer set their own subcat, NOT via CompactionComponentBase), `DeterministicGates`="Deterministic Gates" (Auditor, DetectJson, Resolver, Observer, OutputSnapshot), `Core`, `Signals` (+ the 5 Compositors moved from Utility), `Models` (untouched), `Tools` (+ Router moved from Regulators; ToolsInUse/"Tools Present" retagged Grounding→Tools), `Grounding` (+ Library/ImageGatherer from Resources; the grounders retagged Resources→Grounding), `Regulators` (+ SignalLimiter), `Transmitters` (ComponentTransmitter, PyTransmitter), `Extra` (ZoomGuid, Picker, Deserializer, Serializer). Emptied folders removed: Tokens, Compaction, Perception, GhPython, Resources, Utility, Serializers.
