...
| **Human Tools** | Chat-window affordances for the HUMAN, not the model (`HumanToolComponentBase : PhyBase` — passive emitters: no inputs, one `Param_HumanTool` output; Core union `HumanTool` in `Physalia.Core/HumanTools/`): Geometry Snapshot (arms the chat geometry button; sends a viewport snapshot as its own message), Add Image (enables image paste/drop/picker in the prompt box — image intake is fully disabled without it). Wired into the Conversation Log's Human Tools input; never touch the system prompt, never advertised to the model |
