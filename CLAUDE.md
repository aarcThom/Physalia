...
- **The answer goes two ways, and that is the point of the node.** The full bodies land on the
  **Response** output (LIST access, one item per page fetched) for the definition to parse; what goes
  BACK to the model is `ApiResponseSummary` — record count, *total* matched, field names, one sample
  record. A blind truncation hands the model the first few rows and no hint that more exist, which is
