...
## Latest
- [Harness Builder meta-preset](harness-builder-preset.md) — 2026-09-07, **RUN END TO END** (it generated a preset that then loaded stably): `Files/PRESETS/User/Codex - Harness Builder.gh` writes other harnesses, and its preamble encodes the whole scripted-build contract. The recipe was verified in **run_rhino_script's CPython 3.9** before the preamble was written, not assumed from my IronPython scripts. **FOUR Physalia component names are ambiguous** — `Read PDF` is both an LLM tool and a human tool — so resolve by name AND ribbon section, and check the emitted object has the inputs you are about to wire.
- [ComfyUI render preset](comfy-render-preset.md) — 2026-09-07, **RUN END TO END** (viewport → SDXL img2img → image back in the chat, ~19s on the 4060 Ti): `Files/PRESETS/User/Codex - Rhino to ComfyUI.gh`. **comfy-mcp wraps comfy-cli, so `uvx comfy-mcp` on Windows lists all 39 tools and then fails every call** — it must run in WSL via `wsl.exe -- env PATH=… `, and `--` plus an explicit PATH are both required. Also: **never block the UI thread waiting on `run_rhino_script`** (it marshals to `RhinoApp.Idle`, so a sleeping driver makes the call look like it never ran), and `import System` does not give you `System.Drawing` in Rhino's CPython 3.9.
- [PDF natives verified](pdf-natives-verified.md) — 2026-09-07, **RUN IN RHINO**: `PdfNativeLibrary` resolves, full-page AND region renders are correct (images inspected, not just byte counts), and a placed Read PDF answered `render` with a real `InlineImage`. Two time-sinks: the node reads **`<project>/PDF`**, not the folder you typed, and a manual Construct Tool Call batch **cannot show a call's return value** — reflect `ExecuteCallAsync` instead.
- [MCP bridge + OAuth](mcp-integration.md) — 2026-09-07, **RUN IN RHINO**: the bridge connects a remote Streamable HTTP server (13 tools) beside a local stdio one, and a `tools/call` round-trips. OAuth drives all the way to the browser: **dynamic client registration succeeds**, PKCE S256, loopback callback, RFC 8707 `resource`. Only the token exchange + `FileTokenCache` write remain, and they need a human to press Allow.
- [Blender MCP preset](blender-mcp-preset.md) — 2026-09-07, **RUN LIVE** (cube created in Blender through the pipeline): `Files/PRESETS/User/Codex - Blender.gh` drives Blender over MCP. Three traps, each invisible in a green solve: **the Claude Code provider IGNORES `tools`**, so a Claude Code Model can never drive a Router — check the provider before wiring one; **Rhino's `PYTHONHOME` kills every Python/`uvx` MCP server** (empty it in `env`; the error says `importlib._abc`, nothing about Rhino); and **an input left unwired in a preset FILE grows a fresh Picker on every LOAD** — on Schema it snaps to `values[0]` and folds a foreign JSON schema into the prompt.
