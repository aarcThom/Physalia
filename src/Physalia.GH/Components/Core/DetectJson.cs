...
public class DetectJson : RoutingComponentBase<string>
{
    ...
    /// </summary>
    public DetectJson()
        : base("Detect JSON", "DJson", "Passes responses containing JSON (even malformed) to Success; routes plain conversation to Fail so it never triggers validation feedback.", "Core")
    {
    }
