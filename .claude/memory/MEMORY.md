...
## Latest
- [Group-scoped grounding](group-scoped-grounding.md) — 2026-07-28: master "Physalia" group auto-enrolls every LLM placement; new "Physalia Group Components" grounder exports only its contents (`sha256-group-` checksum prefix selects the patch frame); plug-in tabs unchecked by default in the grounding selector (selection-level, still opt-in-able); prompt/feedback verbosity sweep (schema rules deduped against preambles, mismatch feedback no longer embeds the canvas twice). NOT yet run in Rhino.
