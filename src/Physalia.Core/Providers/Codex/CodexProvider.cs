...
#pragma warning restore IDE0052
        ...
        [EnumeratorCancellation] CancellationToken ct)
    {
        ...
        {
            var ephemeral = new CodexSession(codexConfig.ModelId, codexConfig.ReasoningEffort, systemPrompt.Text);
            try
            {
        ...
        CodexSession session = ResolveSession(sessionKey, codexConfig, systemPrompt.Text);
        ...
        else
        {
            ...
            {
                session = ReplaceSession(sessionKey, codexConfig, systemPrompt.Text);
            }
    ...
    private static CodexSession ResolveSession(Guid sessionKey, CodexConfig config, string systemPrompt)
    {
        lock (_poolLock)
        {
            if (_sessions.TryGetValue(sessionKey, out CodexSession? existing))
            {
                // A changed model, effort or system prompt cannot be applied to a running thread —
                // all three are fixed when the thread is opened.
            ...
            var session = new CodexSession(config.ModelId, config.ReasoningEffort, systemPrompt);
            _sessions[sessionKey] = session;
            return session;
        }
    }

    private static CodexSession ReplaceSession(Guid sessionKey, CodexConfig config, string systemPrompt)
    {
        lock (_poolLock)
        {
