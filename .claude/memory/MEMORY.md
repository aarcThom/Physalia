...
- [Chat token counter](chat-token-counter.md) — bottom-right counter in chat window mirrors the TokenEstimator downstream of the viewed chat's Recorder (VolatileData read + `setTokenCount` bridge verb); hidden when none wired. 2026-07-03, builds+tests clean, live test pending.
- [Detect JSON gate](detect-json-gate.md) — presence gate LLM Call→Schema Validator: attempted JSON (even malformed) → Success; plain chat → Fail quietly (raw text, Remark). New `JsonDetector` in Core/Validation. 2026-07-03, builds+191 tests clean, live test pending.
- [Memory tool](memory-tool.md) — provider-agnostic Anthropic-style `memory` tool (view/create/str_replace/insert/delete/rename); a PLAIN tool (no grounding), reached via `/t/memory/global`//`/t/memory/local`; global + per-`.gh`-file scopes under `Files/memories`. 2026-07-02, builds+tests clean, live test pending.
- [Rhino Geometry tool + /t/ refs](rhino-geometry-tool-and-slash-t.md) — new `create_rhino_geometry` tool (bakes Rhino geo + drops referencing GH param via Idle); Tools moved onto `Instructions.Tools`; Tools Present relocated Tools→Grounding (emits `ToolsGrounding`, wire into Recorder); `/t/<toolname>` prompt refs + purple `/`-command highlighting. **Follow-up: CompTx now reuses those placed params (no duplicates)** via a `physalia.reference` GhJSON node + `CanvasInputGrounder` + registry on the tool, reusing the anchored-placement rewire machinery. 2026-07-01, builds+145 tests clean, live test pending.
...
- [Plain-spoken rename](component-rename-plainspoken.md) — Chatbox→Chat, Composer→System Prompt, Reasoner→LLM Call, Recorder→Conversation Log (2026-07-03, full-depth, GUIDs pinned); ConversationRecorder→ConversationLogBuilder; kept GroundingComposer + Composer.svelte. Builds+198 tests clean.
...
- [GhJSON library is reference-only](ghjson-library-reference-only.md) — local ghjson-dotnet/ghjson-spec repos are third-party reference downloads; NEVER modify, consume via nuget.org (1.1.1+ = doc-level GhPatch + public Delete/Connect/Disconnect canvas primitives; known add-id renumber bug to pre-remap around).
- [Iterative canvas editing](iterative-canvas-editing.md) — canvas-state grounding (CanvasStateGrounder) + ghpatch dual-mode CompTx on GhJSON 1.1.1 (pivots now INT); patch mode edits in place, kills the reopen-duplication bug. Follow-up: CanvasInputGrounding RETIRED — Rhino refs detected doc-side (CanvasRhinoReferences) + physalia.rhinoRef marker in the export; chat page renamed "Referenced Rhino Geometry". 2026-07-07, builds+218 tests clean, live test pending.
...
- [System-prompt preambles](system-prompt-preambles.md) — System Prompt assembles PREAMBLE + SCHEMA from Files/SYSTEM_PROMPTS (only .txt/.json/.yaml resolve, NOT .md). Cleaned to exactly TWO pairs 2026-07-07: `Node Graph` (.txt+.json, oneOf full-doc|ghpatch umbrella) and `Python3 Script` (.txt+.json); minified/PhySchema/GhJSONSchema deleted + dead PhySchema code removed.
- [Obsolete component GUID validation](obsolete-component-guid-validation.md) — LLM placed obsolete colour Multiplication because GhJSON lib's `Put` falls back to unfiltered `CreateByName` (first name match, obsolete included) for unstamped nodes; fix = `GhJsonBridge.StampComponentGuids` stamps non-obsolete GUIDs at placement (self-sufficient w/o Resolver) via shared `ComponentCatalogProvider`. Obscure kept. Verified 2026-06-30.
- [Slider nicknames](slider-nicknames.md) — LLM-placed sliders now get meaningful canvas labels: schema (`nickName` field + rule) + preamble instruct it, `PhySchemaComponent.NickName` carries it, and placement preserves it (ApplyNickNameDisplay no longer clobbers floating-param nicknames). 2026-06-30, builds+tests clean, live test pending.
