...
**Research deliverables (not implemented):** `planning/deterministic-gates.md` (gate catalog; first 3 = Retry Limiter, Python Syntax, Regex/Marker — NOTE: a JSON well-formedness gate is redundant, the Auditor's SchemaValidator already does JsonDocument.Parse + Fails on malformed JSON whenever a schema is wired) and `planning/tool-components.md` (tool catalog; first 5 = get_document_summary, inspect_component, query_geometry, place_graph, run_python). Both ground heavily on existing bases (RoutingComponentBase/SignalLimiter; ToolComponentBase/GhJsonBridge).
