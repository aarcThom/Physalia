...
- **MUST DO — after ANY change to the Svelte UI (`src/Physalia.UI`), build with `dotnet build src/Physalia.slnx -c Debug` (NOT `npm run build` alone) so the rebuilt UI is staged into `bin/Debug`.** `npm run build` only writes `src/Physalia.UI/dist/index.html`; the MSBuild `BuildPhysaliaUI` target then copies it to `Files/UI/chat.html`, and `Physalia.GH` (ProjectReference) stages that into `src/Physalia.GH/bin/<Config>/Files/UI/chat.html`. Skipping the `dotnet build` strands the change in `dist/` and Rhino loads the old UI. Verify the copy landed in `bin/Debug` before reporting the UI change as done. (Build `-c Release` too when targeting the Release output.)
