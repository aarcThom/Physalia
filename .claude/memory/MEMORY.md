...
## Latest
- [Teaching presets](teaching-presets.md) - 2026-09-07, twelve annotated .phy harnesses in wip_presets/, ALL RUN LIVE, generated from tools/presets/. Three provider facts earned the hard way: **Claude Code cannot call tools at all** (it reports "no such tool available" and nothing looks wrong), **Codex's deferral drops a big JSON payload emitted alongside a tool call**, and **a pinned Codex model name 404s** because the CLI's list is live. Plus a real defect: **System Prompt re-places a Picker on every file READ and it snaps to values[0]**, folding the C# preamble into a conversational preset.
