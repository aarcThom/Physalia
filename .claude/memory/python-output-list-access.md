...
**Diagnostic that cracked it (REMOVED 2026-06-29 once resolved):** `PyTransmitter` had a **Remark text output** (index 2) showing, per output, live `access`/`conv`/volatile-data shape AND the compiled signature `access`/`autoDeclare` (reflection `Context→Script→GetCode()→Outputs`). Reading `GH_ObjectWrapper<PyObject>` vs flattened per-element goos was the ground truth at each step. It and its supporting bridge methods (`GetOutputDiagnostics`, `GetCompiledOutputSignature`, `DescribeGoo`, `ReadConverter`) were deleted after the fix landed — the `Remark` output, `RegisterAdditionalOutputs`/`OnSolveTick` overrides, and `BuildOutputDiagnostic` are gone; only the Success/Fail signals remain. If a regression needs re-diagnosing, restore from git history (commit on/after 2026-06-29).
