...
## Latest
- [Model API credentials](model-api-credentials.md) — 2026-09-04, **BUILT**: providers are set up in the chat window and stored DPAPI-encrypted; endpoint+key collapsed into one `GH_ModelApi` wire (OpenAI-compatible node lost Base URL, **new GUID** — the Local LLM preset needs re-saving). **`API_KEY_CONFIG.YAML` deleted outright** — resolution is env var → encrypted store, nothing else. Rules: **encrypt what the machine writes, not what the human writes**, and **availability is not consent** (a found key/CLI is offered, never adopted).
...
- [MCP setup page](mcp-setup-page.md) — 2026-09-04: landing page (paste a `claude`/`codex` CLI command, or fill the form) + `McpCommandParser` in Core; Test connection writes nothing. **CLAUDE.md's MCP section is stale on this.**
