...
metadata: 
  ...
  modified: 2026-08-17T08:29:01.865Z
---
...
**The relationship between the first two is itself a rule (learned the hard way 2026-08-17):** when a
signal carries `ContentBlocks`, they are the **whole** content of the turn and `Payload` is only their
text trace — `ConversationLogBuilder` reads non-empty blocks as authoritative and uses the payload for
tracing alone. So any component that AGGREGATES signals must combine the two together, not
independently: joining payload strings while merely concatenating block lists leaves a text-only
part's text in the payload and in no block, and it is dropped without a warning. `SignalAggregation`
in Core holds that policy for Merge Signal and the Feedback Collector; see [[merge-signal-join]].
