...
**Sections** = the GH **subCategory** string (the C# namespaces are flat `Physalia.GH.Components` — folders are just org). New folders + subCategories: `TokensCompaction`="Tokens & Compaction" (TokenEstimator, TokenizationTechniques, + the 6 compaction comps; note TokenThreshold & Summarizer set their own subcat, NOT via CompactionComponentBase), `Guardrails`="Guardrails" (Schema Validator, Component Resolver, Canvas Observation, Geometry Observation), `Pipeline` (was `Core`, renamed 2026-07-04), `Signals` (+ the 5 Compositors moved from Utility), `Models` (untouched), `Tools` (+ Router moved from Regulators; ToolsInUse/"Tools Present" retagged Grounding→Tools), `Grounding` (+ Component Catalog/Image Sources from Resources; the grounders retagged Resources→Grounding), `Regulators` (+ SignalLimiter, + DetectJson moved from Guardrails; **section renamed `Regulators`→`Control Flow`, folder→`ControlFlow/` on 2026-07-06** — see [[component-rename-plainspoken]]), `Transmitters` (ComponentTransmitter, PyTransmitter), `Extra` (ZoomGuid, Picker, Deserializer, Serializer). Emptied folders removed: Tokens, Compaction, Perception, GhPython, Resources, Utility, Serializers.

**Ordering (SUPERSEDED 2026-07-06 — all `GH_Exposure` tier overrides removed):** Originally each component forced its dropdown group/order via `public override GH_Exposure Exposure => GH_Exposure.<tier>` (tiers primary, secondary, tertiary, `quarternary` [GH's real misspelling — extra r], quinary, senary, septenary, obscure; GH draws a separator between tiers). On 2026-07-06 **all 47 per-component tier overrides were stripped** — every component now falls to the default (`primary`), so each ribbon panel's dropdown is one flat, separator-free group pending a fresh grouping spec. Kept: `PhyParam.Exposure => hidden` (keeps Param_ classes out of the ribbon) and the `ComponentCatalogProvider` `== GH_Exposure.hidden` read. Note `obscure` was also a visibility state — WebSearch/Summarizer were only-in-dropdown and are now visible in the strip. **Tab (subcategory) order itself is NOT API-controllable — GH renders tabs alphabetically.**
