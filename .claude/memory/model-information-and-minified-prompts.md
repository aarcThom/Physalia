...
**Research deliverables (not implemented):** `planning/deterministic-gates.md` (gate catalog). IMPORTANT: many "obvious" gates are already covered by existing components — JSON well-formedness→Auditor (SchemaValidator parses), Python syntax→PyTransmitter (routes target errors), retry/attempt cap→SignalLimiter (within/over count). Revised first 3 (genuinely new) = **Geometry-Valid Gate** (Observer only catches GH errors, not IsValidWithLog), **Allow/Deny Content Filter** (block dangerous Python before PyTransmitter runs it — errors don't catch harmful-but-successful code), **Token Budget Meter** (cumulative spend cap vs SignalLimiter's attempt cap). Next: Dedup gate. and `planning/tool-components.md` (tool catalog; first 5 = get_document_summary, inspect_component, query_geometry, place_graph, run_python). Both ground heavily on existing bases (RoutingComponentBase/SignalLimiter; ToolComponentBase/GhJsonBridge).
