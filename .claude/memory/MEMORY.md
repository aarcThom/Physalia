...
## Latest
- [C# Transmitter](csharp-transmitter.md) — 2026-08-11: new `CsTransmitter` (harness outlet "C#") + its `C# Script` preamble/schema pair. C# declares its params TWICE (submission JSON + the `RunScript` signature), so the push is gated on a signature-agreement guard copying the engine's own regexes verbatim. Every script transmitter must now test `LanguageSpec` — one `IScriptComponent` covers Python 3, IronPython and C#. **Interface Lock widened to any `ScriptTransmitterBase`**, enforcement moved onto that base, per-language prose via the new Core `ScriptInterfaceDialect`. Not yet run in Rhino.
