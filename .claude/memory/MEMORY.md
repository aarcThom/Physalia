...
## Latest
- [Watch Modelling](watch-modelling.md) — 2026-09-06, BUILT not run in Rhino: record what the USER does in Rhino as a repeatable procedure. Built on Rhino's own command events, not a geometry diff; **`BeforeTransformObjects`, not After** (only that one carries the Transform); fires on DISARM; the filtering and undo resolution live in Core and are tested.
- [Trigger tier](trigger-tier.md) — 2026-09-06, BUILT not run in Rhino: Physalia can finally START a round on its own (Timer / Folder Watcher / Rhino Changed / Data Changed). **Arming is session-only and never serialized**, `PipelineWake` stops a wake-up being silently dropped on a harness sub-document, and `PipelineFileWrites` breaks the download→watcher→download loop.
- [Signal conditionals](signal-relay-conditionals.md) — 2026-09-06: `SignalRelayBase` (Gate / Hold / Switch / Throttle) plus For Each. **A relay forwards the ORIGINAL signal** — a re-mint would strip the Instructions a gate on that hop exists to pass through.
...
- [Harness delegation](harness-delegation.md) — 2026-09-06: a harness as a TOOL of another harness, so a subtask stops living in the caller's one conversation. Paired on the inner GH_Document with no wire; one task at a time; refuse up front rather than time out.
- [Session budget + records](session-budget-and-records.md) — 2026-09-06: Budget Guard bounds a SESSION (nothing else did), the conversation autosaves to the project folder and resumes only on request, and `runs.jsonl` logs one line per inference call. A call with no usage still counts as a call.
